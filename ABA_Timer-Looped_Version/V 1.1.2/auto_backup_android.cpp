...
int main()
{
  string ip, choose, answer_1,answer_2, repeat;
  long int time;
  int hour, min, repeat_hour, repeat_min, control;
  int select;
  cout<<"Do you have ADB Driver installed on your pc? (if you don't konw the asnswer insert 'no'): ";
  cin>>answer_1;
  cout<<"Do you want to start ADB as root?";
  cin>>answer_2;
  if (answer_1=="no")
  {
  ...
  else
  {
    ...
    if(choose=="BACKUP"||choose=="Backup"||choose=="backup")
    {
       ...
       if(select==1)
       {
         ...
         {
           cout<<"Input the hour in 24 format when you want the program to repeat";
           cout<<"Hour: ";
           cin>>repeat_hour;
           cout<<"Minutes: ";
           cin>>repeat_min;
           ...
           while(true)
           {
             time=(86400000-(((hour*3600)+(min*60))*1000))+((repeat_hour*3600)+(repeat_min*60)*1000);
             control=control+1;
             if(control>1)
             {
               time=86400000;
